<template>
    <div>
                ...
                <nav class="otimize">
                    <div class="nav-wrapper">
                        <div class="col s12">
                            ...
                            <a href="!#" class="breadcrumb">Cargos</a>
                        </div>
                    </div>
                </nav>
            </div>
        ...
        <div class="row">
            <div class="col s12">
                ...
                <div class="card-panel">
                    <section>
                        <div class="row">
                            <h4 class="col xl6 l6 m6 s12"><i class="fas fa-user-tag small"></i> Lista de cargos cadastrados</h4>
                            <div class="col xl6 l6 m6 s12" v-if="empresaSelecionadaId != null && empresaSelecionadaId != 0">
                                <a class="btn-floating indigo darken-2 waves-effect waves-light right"
                                   ...
                                   :title="empresaSelecionada.departamentos.length == 0 ? 'Não é possível cadastrar novos cargos pois esta empresa não possui departamentos cadastrados' : 'Cadastrar novo cargo'"
                                   @click="cadastraNovoCargo"><i class="material-icons left">add</i></a>
                            </div>
                        </div>
                    </section>
                    ...
                    <section>
                        ...
                        <div class="row" v-if="usuarioLogado.perfil.super_user == 1 && !retornaSeTemEmpresasCadastradas">
                            <div class="col s12">
                                <md-empty-state
                                    ...
                                    md-description="Para poder cadastrar os cargos é necessário primeiro cadastrar uma empresa.">
                                </md-empty-state>
                            </div>
                        </div>
                        ...
                        <div class="row" v-else-if="!usuarioLogado.perfil.super_user == 1 && !retornaSeTemEmpresasCadastradas">
                            <div class="col s12">
                                <md-empty-state
                                    ...
                                    md-icon="error"
                                    md-label="Desculpe! Não é possível continuar a operação"
                                    md-description="Infelizmente você não pode cadastrar nenhum cargo já que seu usuário não está associado a nenhuma empresa. Solicite ao seu administrador que o associe a alguma empresa.">
                                </md-empty-state>
                            </div>
                        </div>
                        ...
                        <div v-else>
                            ...
                            <md-empty-state
                                v-if="!loadingsAtivos && empresaSelecionada != null && empresaSelecionada != undefined && cargosCadastrados.length == 0"
                                md-icon="../img/svg/icons/user-tag-solid.svg"
                                :md-label="empresaSelecionada == null || empresaSelecionada == undefined || !empresaSelecionada.ativo ? 'Não há cargos cadastrados e nem é possível cadastrar novos cargos pois a empresa encontra-se inativa.' : 'Não há cargos cadastrados até o momento'"
                                :md-description="empresaSelecionada == null || empresaSelecionada == undefined || !empresaSelecionada.ativo ? '' : 'Cadastre uma novo cargo para que possamos dar prosseguimento aos próximos cadastros.'">
                                <a class="btn indigo darken-2 waves-effect waves-light"
                                   v-if="empresaSelecionada != null && empresaSelecionada != undefined && empresaSelecionada.ativo"
                                   :disabled="empresaSelecionada.departamentos.length == 0"
                                   :title="empresaSelecionada.departamentos.length == 0 ? 'Não é possível cadastrar novos cargos pois esta empresa não possui departamentos cadastrados' : 'Cadastrar novo cargo'"
                                   @click="cadastraNovoCargo">
                                    <i class="material-icons left">add</i>Cadastrar novo cargo
                                </a>
                            </md-empty-state>

                            <p v-else-if="loadingTextualBuscaCargos" class="label-carregamento">Carregando cargos cadastrados...</p>

                            <div v-else-if="cargosCadastrados.length > 0">
                                <div class="row">
                                    <div class="input-field col s12">
                                ...
                                <div class="row">
                                    <div class="col s12">
                                        <table class="responsive-table striped">
                                            ...
                                            <tr>
                                                ...
                                                <th>
                                                    ...
                                                    <a class="left" href="#" @click="toggleOrder($event, 'nome')">
                                                        <i class="material-icons" v-bind:class="{ 'red-text': filtro.sortPropertyCargo }">sort</i>
                                                    </a>
                                                </th>
                                                <th>
                                                    <span>Status</span>
                                            ...
                                            <tbody>
                                            <tr v-for="(cargo, cargo_i) in listaFiltrada">
                                                <td v-if="usuarioLogado.super_user">
                                                    <span v-if="!cargo.departamento.empresa.ativo">
                                                        <strike><i>{{cargo.departamento.empresa.razao_social}}</i></strike>
                                                        &nbsp;&nbsp;
                                                        <span data-badge-caption=""
                                                              class="new badge red">Excluído</span>
                                                    </span>
                                                    <span v-else>{{cargo.departamento.empresa.razao_social}}</span>
                                                </td>
                                                <td>
                                                    <span v-if="!cargo.departamento.ativo">
                                                        <strike><i>{{cargo.departamento.nome}}</i></strike>
                                                        &nbsp;&nbsp;
                                                        <span data-badge-caption=""
                                                              class="new badge red">Excluído</span>
                                                    </span>
                                                    <span v-else>{{cargo.departamento.nome}}</span>
                                                </td>
                                                <td><span>{{cargo.nome}}</span></td>
                                                <td>
                                                    <span data-badge-caption=""
                                                          class="new badge"
                                                          :class="{'red' : cargo.ativo == 0, 'green' : cargo.ativo == 1}">{{cargo.ativo == 0 ? 'Excluído' : 'Ativo'}}</span>
                                                </td>
                                                <td><span>{{cargo.data_cadastro}}</span></td>
                                                <td v-if="empresaSelecionada != null && empresaSelecionada.ativo">
                                                    <div v-if="cargo.ativo">
                                                        <a style="cursor: pointer;"
                                                           data-toggle="tooltip"
                                                           data-placement="top"
                                                           title="Alterar"
                                                           @click="editaCargo(cargo)">
        ...
        <vue-cadastra-edita-cargos ref="cadastra_edita_cargo"
                                   @emissaoDeDadosSalvos="concluiSalvamentoDeDados"></vue-cadastra-edita-cargos>
    </div>
</template>
    ...
    export default {
        ...
        data() {
            return {
                ...
                loadingTextualBuscaCargos: false,
        ...
        computed: {
            listaFiltrada: function () {
                ...
                if(this.departamentoSelecionadoId != null && this.departamentoSelecionadoId != undefined && this.departamentoSelecionadoId != 0)
                    self.filtro.listaOrdenada = _.filter(self.cargosCadastrados, (cargo) => { return cargo.departamento.id == this.departamentoSelecionadoId })
                else
                    self.filtro.listaOrdenada = self.cargosCadastrados

                self.filtro.listaOrdenada = _.orderBy(self.filtro.listaOrdenada.filter(function (cargo) {
                    var searchRegex = new RegExp(self.filtro.filterTerm, 'i')
                    return (
                        searchRegex.test(cargo.departamento.empresa.razao_social) ||
                        searchRegex.test(cargo.departamento.nome) ||
                        searchRegex.test(cargo.nome) ||
                        searchRegex.test(cargo.ativo == 1 ? 'Ativo' : 'Inativo') ||
                        searchRegex.test(cargo.data_cadastro)
                    )
            ...
            loadingsAtivos(){
                return (this.loadingTextualBuscaCargos || this.loading)
            },
        },
        watch: {
        ...
        methods: {
            ...
            listaDepartamentosCadastrados() {
                ...
                if(this.empresaSelecionadaId == null || this.empresaSelecionadaId == 0){
                    this.$refs.alert.abreMensagens({status_resposta: 'error', msg: 'É obrigatório selecionar uma empresa válida para listar os cargos cadastrados.'})
                    return
                }
            ...
            listaCargosCadastrados() {
                ...
                this.loadingTextualBuscaCargos = true

                axios.post('cargos/lista/por/empresa/json', request)
                    .then((response) => {
                        this.cargosCadastrados = response.data;
                        this.loadingTextualBuscaCargos = false;
                    }).catch(error => {
                        this.empresaSelecionadaId = null;
                        this.$refs.alert.abreMensagens(error.response);
                        this.loadingTextualBuscaCargos = false;
            ...
            cadastraNovoCargo(){
                if(this.empresaSelecionada == null || this.empresaSelecionada === 'undefined')
                    Swal.fire('Oops!', 'Não é possível cadastrar um cargo sem antes selecionar uma empresa.','error')
                else{
                    if(this.empresaSelecionada.departamentos.length == 0)
                        Swal.fire('Perae!', 'Não é possível cadastrar um cargo em uma empresa sem departamentos cadastrados.', 'error')
                    else {
                        ...
                        if(departamentosAtivos == undefined || departamentosAtivos == null || departamentosAtivos.length == 0)
                            Swal.fire('Perae!', 'Não é possível cadastrar um cargo pois os departamentos da empresa foram todos excluídos.', 'error')
                        else
                            this.$refs.cadastra_edita_cargo.cadastraNovoCargo(this.empresaSelecionada, departamentosAtivos)
                    }


                }
            },
            editaCargo(cargoParaEdicao){
                let departamentosAtivos = _.filter(this.empresaSelecionada.departamentos, (departamento) => {
                    return departamento.ativo
                })

                if(departamentosAtivos == undefined || departamentosAtivos == null || departamentosAtivos.length == 0)
                    Swal.fire('Perae!', 'Não é possível editar cargo pois os departamentos da empresa foram todos excluídos.', 'error')
                else
                    this.$refs.cadastra_edita_cargo.editaCargo(cargoParaEdicao, departamentosAtivos)
            },
            ...
            deletaCargo(cargoParaExclusao){
                if(confirm('Tem certeza que seja excluir esse cargo?')){
                    this.$refs.alert.limpaMensagens()
                    this.loading = true

                    axios.delete('/cargos/' + cargoParaExclusao.id).then(response => {
                        this.loading = false
                        this.listaCargosCadastrados()
            ...
            reativaCargo(cargoParaReativar){
                if(confirm('Tem certeza que seja reativar esse cargo?')){
                    this.$refs.alert.limpaMensagens()
                    this.loading = true

                    axios.get('/cargos/reativa/' + cargoParaReativar.id).then(response => {
                        this.loading = false
                        this.listaCargosCadastrados()
                        this.$refs.alert.abreMensagens(response)
                        Swal.fire('Ótimo', response.data.msg, 'success')
            ...
            iconsColor(field, value) {
                switch (field) {
                    ...
                    case 'departamento.nome':
                        ...
                        break;
                    case 'nome':
                        this.filtro.sortPropertyCargo = value
                        break;
                    case 'ativo':
